...
import gymnasium as gym  # 修改：导入 gymnasium
import numpy as np
from gymnasium import spaces  # 修改：使用 gymnasium 的 spaces
from gymnasium.utils import seeding  # 修改：使用 gymnasium 的 seeding

# 修改：导入 gymnasium 兼容的多智能体空间
...
class Combat(gym.Env):
    ...
    also sees other agents in its visual range (5 × 5 surrounding area). The rewards are given agent-wise:
    ...
    """
    metadata = {'render_modes': ['human', 'rgb_array'], 'render_fps': 4}  # 修改：添加 render_fps

    def __init__(self, grid_shape=(15, 15), n_agents=5, n_opponents=5, init_health=3, full_observable=False,
                 step_cost=0, max_steps=100, render_mode=None):  # 修改：添加 render_mode 参数
        super().__init__()  # 修改：调用父类初始化
        ...
        self.render_mode = render_mode  # 修改：保存 render_mode
        ...
        self.window = None  # 修改：使用 window 替代 viewer
        self.clock = None
        ...
        # 修复：使用 float32 类型避免警告
        self._obs_low = np.repeat([-1., 0., 0., -1., 0., 0.], 5 * 5).astype(np.float32)
        self._obs_high = np.repeat([1., n_opponents, init_health, 1., 1., 1.], 5 * 5).astype(np.float32)
    ...
    def __init_full_obs(self):
        ...
        for opp_i in range(self._n_opponents):
            pos = [int(opp_pos_index[opp_i] / 5) + opp_team_corner[0], opp_pos_index[opp_i] % 5 + opp_team_corner[1]]
            while True:
                if self._full_obs[pos[0]][pos[1]] == PRE_IDS['empty']:
    ...
    def reset(self, seed=None, options=None):  # 修改：添加 seed 和 options 参数
        # 修改：使用 gymnasium 的 seeding
        if seed is not None:
        ...
        # 修改：返回 observations 和 info
        observations = self.get_agent_obs()
        info = {}
        
        return observations, info  # 修改：返回两个值

    def render(self):
        """ 修改：更新渲染方法以兼容 Gymnasium """
        ...
        if self.render_mode == 'rgb_array':
            ...
            if self.window is None:
                pygame.init()
                pygame.display.init()
                self.window = pygame.display.set_mode((self._grid_shape[1] * CELL_SIZE, 
                                                     self._grid_shape[0] * CELL_SIZE))
                pygame.display.set_caption("Combat Environment")
    ...
    def step(self, agents_action):
        ...
        if self._step_count >= self._max_steps:
            truncated = True
        # 检查是否所有对手死亡或所有己方死亡（终止）
        elif (sum([v for k, v in self.opp_health.items()]) == 0) or (sum([v for k, v in self.agent_health.items()]) == 0):
            terminated = True
            if (sum([v for k, v in self.opp_health.items()]) == 0):
                win = True

        # 修改：更新 _agent_dones 为 terminated 状态
        self._agent_dones = [terminated or truncated for _ in range(self.n_agents)]
        ...
        info = {
            ...
            'opponent_health': self.opp_health.copy(),
...
# 修改：添加 DependencyNotInstalled 异常类
